...
```{python}
f = lambda x:  x ** 2+22*x+117

x = np.linspace(-14, -8, 500)
y = x ** 2+22*x+117
...
plt.title(r"Gráfica de la función $f(x)=x^2+22x+117$", fontsize= 14)
plt.xlabel("x")
plt.ylabel("y")
plt.grid()
...
```{python}
raiz_1, info_biseccion1 = optimize.bisect(f, -14, -12, full_output=True)
...
```{python}
der_f = lambda x: 2* x **2 + 22*x +117

raiz_1newton, info_newton1 = optimize.bisect(f,-14, -12, full_output=True)
...
```{python}
raiz_1secante, info_secante1 = optimize.newton(f, x0= -14, x1= -12, full_output= True)
...
Consideremos $f(x)=x^3+21x^2+131x+231$
...
f = lambda x:  x ** 3+21*x ** 2+131*x+231

x = np.linspace(-12, 0, 500)
y = x ** 3+21*x ** 2+131*x+231
...
```{python}
raiz_1, info_biseccion1 = optimize.bisect(f, -12, -10, full_output=True)
...
```{python}
der_f = lambda x: 3* x **2 + 42*x +131

raiz_1newton, info_newton1 = optimize.bisect(f,-12, -10, full_output=True)
print("información convergencia método de Newton primera raiz")
print(info_newton1)
print("-----------------")
print(f"La primera raiz es {raiz_1newton}")
...
```{python}
raiz_1secante, info_secante1 = optimize.newton(f, x0= -12, x1= -10, full_output= True)
